...
<body>
	...
	<div id="option-box">
		<form action="/searchCarKeyword">
			<table class="table"
				style="background-color: rgb(238, 238, 238); text-align: center; margin: 0 auto;">
				<tr>
					<th>지역</th>
					<td><select style="width: 50px;" id="zone" name="zone">
					...
					<td><input type="date" name="date-range1" id="date-range1"
						value="날짜입력해주세요" onclick="datePick();"> <span
						style="margin-left: 5px; margin-right: 5px"> ~ </span> <input
						type="date" name="date-range2" id="date-range2"
						onclick="datePick();"></td>
					<th>차종</th>
					<td><label><input type="checkbox" class="carType"
							name="car-type" value="소형" onclick="doOpenCheck(this);">소형</label>
						<label><input type="checkbox" name="car-type"
							class="carType" value="중형" onclick="doOpenCheck(this);">중형</label>
						<label><input type="checkbox" name="car-type"
							class="carType" value="대형" onclick="doOpenCheck(this);">대형</label></td>
					<th>차 이름</th>
			...
			<tr class="carlist">
				...
				<td>
					<button class="btn btn-info btn-sm"
						onClick="location.href='/carDetailView?carNo=<%=c.getCarNo()%>'">상세보기</button>
			</tr>
			<%
				}
			%>
	...
	<script>
		...
		$(document).ready(
				function() {
					...
					function start() {
						console.log("123");
						if (carType != null && Sdate != null && Edate != null
								&& addr != null && carName != null) {
							console.log("start");
							doAjax();
						}
					}
