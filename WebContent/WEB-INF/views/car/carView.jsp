...
<script type="text/javascript"
	src="https://openapi.map.naver.com/openapi/v3/maps.js?ncpClientId=hdnph5ozc8&submodules=geocoder"></script>
<style>
section {
	width: 100%;
}
...
<body>
	...
	<div>
		<div class="index">
			<hr>
			<a href="/carList?reqPage=1">목록으로</a>
			<hr>
		</div>
		<div class="div-left">
			<div class="info-wrapper">
				...
				<div
					style="width: 100%; text-align: center; padding: 10px; margin-top: 30px;">
					...
					<div class="img-list">
						...
						<div>
							<img src="/img/car/<%=c.getCarImage2()%>">
						</div>
						<div>
							<img src="/img/car/<%=c.getCarImage3()%>">
						</div>
						<div>
							<img src="/img/car/<%=c.getCarImage4()%>">
						</div>
					</div>
				</div>
				<table border="1">
			...
			<div id="review-wrapper">
				...
				<h4 style="margin-left: 20px;">
					...
					if (!rList.isEmpty()) {
				%>
					<span class="toggle-review"><img
						src="/img/car/arrow-down.png" width="20"
						style="transition-duration: 0.2s;"></span>
					<%
						}
					%>
				</h4>
					...
					<li class="review-li">
						<div>
							<div style="margin-bottom: 10px">
								<span style="font-size: 15px"><%=r.getRevWriter()%>님</span> <span
									style="font-size: 12px; maring-left: 10px; float: right;"><%=r.getRevEnrollDate()%></span>
							</div>
							<div style="">
								<%
		...
		<div class="div-right">
			<div class="reserve-box">
				...
				<form action="#" method="post" style="">
					...
					<input type="date" class="form-control" id="rsvStartDate"
						name="rsvStartDate" onclick="datePick()">
					...
					<input type="time" class="form-control" name="rsvStartTime"
						id="rsvStartTime" value="10:00:00">
					...
					<input type="date" class="form-control" name="rsvEndDate"
						id="rsvEndDate" onclick="datePick()">
					...
					<input type="time" class="form-control" name="rsvEndTime"
						id="rsvEndTime" value="19:00:00">
	...
	<script>
		...
		window.onload = function() {
			...
			naver.maps.Service.geocode({//주소->위경도 변환하는 객체
				...
				var marker = new naver.maps.Marker({
					...
					map : map
				//화면에 여러 지도가 있을 수 있으니 어느 지도에 표시할지 설정
				});

				var contentString = [//정보창 내용 작성
				'<div class="iw_inner">', '	<p>' + carLoc[0].value + '</p>',
						'</div>' ].join('');
				//지도 이벤트 추가
		...
		$(function() {
			...
			console.log("time : " + _today.getHours());

			console.log("time : " + _today.getMinutes());
			$("#rsvStartDate").val(
			...
			$("#rsvEndTime")
					.change(
							function() {
								...
								if (($("#rsvEndDate").val() <= $(
										"#rsvStartDate").val())
										&& (Etime <= $("#rsvStartTime").val())) {
									alert('출발일보다 더 이른날짜를 선택불가');

									$("#rsvEndTime").val('');
								}
								console.log("realEdate : "
										+ $("#rsvEndDate").val());
								console.log("realSdate:"
										+ $("#rsvStartDate").val());
								console.log("realEtime:"
										+ $("#rsvEndTime").val());
								console.log("realStime : "
										+ $("#rsvStartTime").val());
								hour();
							});
			hour()
		...
		function hour() {
			var start = new Date($("#rsvStartDate").val() + " "
					+ $("#rsvStartTime").val());
			var end = new Date($("#rsvEndDate").val() + " "
					+ $("#rsvEndTime").val());
